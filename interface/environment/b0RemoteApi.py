...
class RemoteApiClient:
    ...
    def simxCallScriptFunction(self,funcAtObjName,scriptType,arg,topic):                    #调用脚本
    ...
    def simxGetObjectPosition(self,objectHandle,relObjHandle,topic):                         #获取位置
        reqArgs = [objectHandle,relObjHandle]
        return self._handleFunction('GetObjectPosition',reqArgs,topic)
    def simxGetObjectOrientation(self,objectHandle,relObjHandle,topic):                     #获取方位
        reqArgs = [objectHandle,relObjHandle]
    ...
    def simxGetObjectPose(self,objectHandle,relObjHandle,topic):                           #获取位姿
        reqArgs = [objectHandle,relObjHandle]
        return self._handleFunction('GetObjectPose',reqArgs,topic)
    def simxGetObjectMatrix(self,objectHandle,relObjHandle,topic):                        #获取旋转矩阵
        reqArgs = [objectHandle,relObjHandle]
        return self._handleFunction('GetObjectMatrix',reqArgs,topic)
    def simxSetObjectPosition(self,objectHandle,relObjHandle,position,topic):             #设置位置
        reqArgs = [objectHandle,relObjHandle,position]
        return self._handleFunction('SetObjectPosition',reqArgs,topic)
    def simxSetObjectOrientation(self,objectHandle,relObjHandle,euler,topic):             #设置方位
    ...
    def simxSetObjectPose(self,objectHandle,relObjHandle,pose,topic):                     #设置位姿
        reqArgs = [objectHandle,relObjHandle,pose]
        return self._handleFunction('SetObjectPose',reqArgs,topic)
    def simxSetObjectMatrix(self,objectHandle,relObjHandle,matr,topic):                  #设置旋转矩阵
        reqArgs = [objectHandle,relObjHandle,matr]
        return self._handleFunction('SetObjectMatrix',reqArgs,topic)
    def simxClearFloatSignal(self,sigName,topic):                                        #下面为清除、设置、获得各类型的信号
        reqArgs = [sigName]
    ...
    def simxStartSimulation(self,topic):                                                                   #开始仿真
        reqArgs = [0]
        return self._handleFunction('StartSimulation',reqArgs,topic)
    def simxStopSimulation(self,topic):                                                                    #结束仿真
        reqArgs = [0]
        return self._handleFunction('StopSimulation',reqArgs,topic)
    def simxPauseSimulation(self,topic):                                                                   #暂停仿真
        reqArgs = [0]
        return self._handleFunction('PauseSimulation',reqArgs,topic)
    def simxGetVisionSensorImage(self,objectHandle,greyScale,topic):                                      #获取视觉传感器彩色图或灰度图
        reqArgs = [objectHandle,greyScale]
        return self._handleFunction('GetVisionSensorImage',reqArgs,topic)
    def simxSetVisionSensorImage(self,objectHandle,greyScale,img,topic):                                  #修改视觉传感器图象
        reqArgs = [objectHandle,greyScale,img]
        return self._handleFunction('SetVisionSensorImage',reqArgs,topic)
    def simxGetVisionSensorDepthBuffer(self,objectHandle,toMeters,asByteArray,topic):                     #获取视觉传感器深度图
    ...
    def simxReadProximitySensor(self,handle,topic):                                            #读取红外传感器状态
        reqArgs = [handle]
        return self._handleFunction('ReadProximitySensor',reqArgs,topic)
    def simxCheckProximitySensor(self,handle,entity,topic):                                    #检查红外传感器状态
        reqArgs = [handle,entity]
        return self._handleFunction('CheckProximitySensor',reqArgs,topic)
    def simxReadForceSensor(self,handle,topic):                                                #读取力传感器状态
        reqArgs = [handle]
    ...
    def simxSetStringParameter(self,paramId,paramVal,topic):                                   #修改字符串变量的参数
        reqArgs = [paramId,paramVal]
        return self._handleFunction('SetStringParameter',reqArgs,topic)
    def simxSetFloatParameter(self,paramId,paramVal,topic):                                    #修改浮点型变量的参数
        reqArgs = [paramId,paramVal]
        return self._handleFunction('SetFloatParameter',reqArgs,topic)
    def simxSetArrayParameter(self,paramId,paramVal,topic):                                    #修改矩阵参数
        reqArgs = [paramId,paramVal]
        return self._handleFunction('SetArrayParameter',reqArgs,topic)
    def simxSetIntParameter(self,paramId,paramVal,topic):                                      #修改整型变量的参数
        reqArgs = [paramId,paramVal]
        return self._handleFunction('SetIntParameter',reqArgs,topic)
    def simxSetBoolParameter(self,paramId,paramVal,topic):                                     #修改bool型变量的参数
        reqArgs = [paramId,paramVal]
        return self._handleFunction('SetBoolParameter',reqArgs,topic)
    def simxGetStringParameter(self,paramId,topic):                                            #获取字符串变量的参数
        reqArgs = [paramId]
        return self._handleFunction('GetStringParameter',reqArgs,topic)
    def simxGetFloatParameter(self,paramId,topic):                                             #获取浮点型变量的参数
        reqArgs = [paramId]
        return self._handleFunction('GetFloatParameter',reqArgs,topic)
    def simxGetArrayParameter(self,paramId,topic):                                             #获取矩阵参数
        reqArgs = [paramId]
        return self._handleFunction('GetArrayParameter',reqArgs,topic)
    def simxGetIntParameter(self,paramId,topic):                                               #获取整型变量的参数
        reqArgs = [paramId]
        return self._handleFunction('GetIntParameter',reqArgs,topic)
    def simxGetBoolParameter(self,paramId,topic):                                              #获取bool型变量的参数
        reqArgs = [paramId]
    ...
    def simxGetJointForce(self,jointHandle,topic):                                       #以下均为关节操作
        reqArgs = [jointHandle]
    ...
    def simxGetObjectChild(self,objectHandle,index,topic):                                #以下为父子类物体操作
    ...
    def simxGetObjectFloatParameter(self,objectHandle,parameterID,topic):                    #获取物体的浮点型变量的参数
        reqArgs = [objectHandle,parameterID]
        return self._handleFunction('GetObjectFloatParameter',reqArgs,topic)
    def simxGetObjectIntParameter(self,objectHandle,parameterID,topic):                      #获取物体的整型变量的参数
        reqArgs = [objectHandle,parameterID]
        return self._handleFunction('GetObjectIntParameter',reqArgs,topic)
    def simxGetObjectStringParameter(self,objectHandle,parameterID,topic):                   #获取物体的字符串类型变量的参数
        reqArgs = [objectHandle,parameterID]
        return self._handleFunction('GetObjectStringParameter',reqArgs,topic)
    def simxSetObjectFloatParameter(self,objectHandle,parameterID,parameter,topic):          #修改物体的浮点型变量的参数
        reqArgs = [objectHandle,parameterID,parameter]
        return self._handleFunction('SetObjectFloatParameter',reqArgs,topic)
    def simxSetObjectIntParameter(self,objectHandle,parameterID,parameter,topic):            #修改物体的整型变量的参数
        reqArgs = [objectHandle,parameterID,parameter]
        return self._handleFunction('SetObjectIntParameter',reqArgs,topic)
    def simxSetObjectStringParameter(self,objectHandle,parameterID,parameter,topic):         #修改物体的字符串类型变量的参数
        reqArgs = [objectHandle,parameterID,parameter]
        return self._handleFunction('SetObjectStringParameter',reqArgs,topic)
    def simxGetSimulationTime(self,topic):                                                   #获取仿真时间
        reqArgs = [0]
        return self._handleFunction('GetSimulationTime',reqArgs,topic)
    def simxGetSimulationTimeStep(self,topic):                                               #获取仿真步长
        reqArgs = [0]
    ...
    def simxGetSimulationState(self,topic):                                                  #获取仿真状态
        reqArgs = [0]
